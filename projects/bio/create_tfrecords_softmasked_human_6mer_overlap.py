"""
Script to create TFRecords with overlapping 6-mer tokenization for softmasked human genome.

Usage: python3 softmasked_human_6mer_overlap.py --bucket-name minformer_data
"""
...
def tokenize_sequence_overlapping(sequence, k=6):
    """
    Tokenize a DNA sequence using overlapping k-mers.
    ...
    # Slide through sequence with window of size k
...
def parse_args():
    parser = argparse.ArgumentParser(description="Create 6-mer tokenized TFRecords for softmasked human genome")
    ...
    parser.add_argument("--max-tokens", type=int, default=8192, help="Maximum number of tokens per sequence")
...
def process_dataframe(df, output_dir, batch_size=128, max_tokens=8192):
    """
    Process DataFrame and create TFRecord files with 6-mer tokenization.
    """
        ...
        if len(batch_sequences) >= batch_size:
            ...
            with tf.io.TFRecordWriter(local_temp) as writer:
                for batch_row in batch_sequences:
                    ...
                    # Tokenize with overlapping 6-mers
                    tokens, lowercase_mask = tokenize_sequence_overlapping(seq, k=6)
...
def main():
    ...
    output_dir = f"gs://{args.bucket_name}/human_softmasked_6mer_overlap/tfrecords/"
