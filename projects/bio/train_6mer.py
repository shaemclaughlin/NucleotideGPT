"""
Training script for 6-mer tokenized models with GCS checkpoint saving.

Usage:
python3 train_6mer.py --checkpoint_dir=gs://minformer_data/checkpoints/6mer_model --max_seq_len=8192 --vocab_size=4098 --log_every=10
"""
...
import data_6mer  # Our new 6-mer data loader
import jax
...
def parse_args():
    parser = argparse.ArgumentParser(description="DNA 6-mer Training Script")
    ...
    parser.add_argument("--max_seq_len", type=int, default=8192, help="Maximum sequence length in tokens")
    ...
    parser.add_argument("--log_dir", type=str, default="/tmp/logs/6mer", help="Base directory for TensorBoard logs")
    parser.add_argument(
        "--checkpoint_dir", 
        type=str, 
        default="gs://minformer_data/checkpoints/6mer_model", 
...
def main():
    ...
    if not args.checkpoint_dir.startswith("gs://"):
        ...
        print("Example: gs://minformer_data/checkpoints/6mer_model")
        # You can either error out or create a default GCS path
    ...
    wandb.init(
        ...
        config={
            ...
            "tokenization": "6-mer overlapping",
    ...
    log_dir_name = f"6mer_d{args.d_model}_l{args.num_layers}_h{args.query_heads}_lr{args.max_lr}_{timestamp}"
    log_dir = os.path.join(args.log_dir, log_dir_name)

    # Data loading setup for 6-mer tokenized human softmasked genome
    print("Loading 6-mer tokenized data from GCS...")
    print("Data path: gs://minformer_data/human_softmasked_6mer_overlap/tfrecords//")
    
    iter = data_6mer.create_iterator(
        file_pattern="gs://minformer_data/human_softmasked_6mer_overlap/tfrecords/record_*.tfrecord",
        batch_size=args.batch_size,
        shuffle=True
    )
    process_batch = data_6mer.process_batch_6mer
    ...
    cfg = model.Config(
        ...
        max_seq_len=args.max_seq_len,  # Still 8192 (we pad to this)
        causal=True,
    ...
    print("="*60)
    print(f"Tokenization: 6-mer overlapping")
    ...
    if args.resume_from_checkpoint:
        ...
        print("Initializing new weights for 6-mer model...")
        weights = model.Weights.init(cfg, jax.random.PRNGKey(0), cfg.mesh, model.fsdp_rules)
        opt_state = model.init_optimizer_state(weights)
        start_step = 0
    ...
    with SummaryWriter(log_dir) as writer:
        ...
        for i in range(start_step, cfg.total_steps):
            ...
            if i % 5000 == 0 and i > 0:  # Every 5000 steps instead of 1000
                ...
                print("\n" + "-"*60)
                print("Token distribution analysis (6-mer):")
